...
tabindex:"-1"});b(k).addClass("ui-state-highlight").attr({"aria-selected":"true",tabindex:"0"});if(q){b("#"+b.jgrid.jqID(c.p.selrow),"#"+b.jgrid.jqID(q)).removeClass("ui-state-highlight");b("#"+b.jgrid.jqID(d),"#"+b.jgrid.jqID(q)).addClass("ui-state-highlight")}h=true;c.p.selrow=k.id;}else {b(c.rows.namedItem(c.p.selrow)).removeClass("ui-state-highlight").attr({"aria-selected":"false",
tabindex:"-1"});if(q){b("#"+b.jgrid.jqID(c.p.selrow),"#"+b.jgrid.jqID(q)).removeClass("ui-state-highlight");}h=false;c.p.selrow=null;}c.p.onSelectRow&&i&&c.p.onSelectRow.call(c,k.id,h)}}}})},resetSelection:function(d){return this.each(function(){var i=this,g,c;if(typeof d!=="undefined"){c=d===i.p.selrow?i.p.selrow:d;b("#"+b.jgrid.jqID(i.p.id)+
...
b.extend({onEnter:null,onSpace:null,onLeftKey:null,onRightKey:null,scrollingRows:true},d||{});return this.each(function(){var g=this;b("body").is("[role]")||b("body").attr("role","application");g.p.scrollrows=i.scrollingRows;b(g).keydown(function(c){var h=b(g).find("tr[tabindex=0]")[0],k,l,a,q=g.p.treeReader.expanded_field;if(h){a=g.p._index[h.id];if(c.keyCode===37||c.keyCode===38||c.keyCode===39||c.keyCode===40){if(c.keyCode===38){c.preventDefault();c.stopPropagation();l=h.previousSibling;k="";if(l)if(b(l).is(":hidden"))for(;l;){l=l.previousSibling;
if(!b(l).is(":hidden")&&b(l).hasClass("jqgrow")){k=l.id;break}}else k=l.id;b(g).jqGrid("setSelection",k)}if(c.keyCode===40){c.preventDefault();c.stopPropagation();l=h.nextSibling;k="";if(l)if(b(l).is(":hidden"))for(;l;){l=l.nextSibling;if(!b(l).is(":hidden")&&b(l).hasClass("jqgrow")){k=l.id;break}}else k=l.id;b(g).jqGrid("setSelection",k)}if(c.keyCode===37){g.p.treeGrid&&g.p.data[a][q]&&b(h).find("div.treeclick").trigger("click");b.isFunction(i.onLeftKey)&&i.onLeftKey.call(g,g.p.selrow)}if(c.keyCode===39){g.p.treeGrid&&!g.p.data[a][q]&&
...
b.p._complete=function(){a("#"+a.jgrid.jqID(b.p.id)+"_frozen").remove();jQuery(b.grid.fbDiv).height(jQuery(b.grid.bDiv).height());var h=a("#"+a.jgrid.jqID(b.p.id)).clone(true);a("tr",h).each(function(){a("td:gt("+k+")",this).remove()});a(h).width(1).attr("id",a.jgrid.jqID(b.p.id)+"_frozen");a(b.grid.fbDiv).append(h);if(b.p.hoverrows===true){a("tr.jqgrow",h).hover(function(){a(this).addClass("ui-state-hover");a("#"+a.jgrid.jqID(this.id),"#"+a.jgrid.jqID(b.p.id)).addClass("ui-state-hover")},function(){a(this).removeClass("ui-state-hover");
