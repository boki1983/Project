var bMoveable=false;  //設定日曆是否可以拖動
...
strFrame+='BORDER-BOTTOM: #ff9900 1px solid;BACKGROUND-COLOR: #fff8ec;font-family:細明體;}';
strFrame+='TD{FONT-SIZE: 9pt;font-family:細明體;}';
strFrame+='</style>';
strFrame+='<scr' + 'ipt>';
strFrame+='var datelayerx,datelayery; /*存放日曆控制項的鼠標位置*/';
strFrame+='var bDrag; /*標記是否開始拖動*/';
strFrame+='function document.onmousemove() /*在鼠標移動事件中，如果開始拖動日曆，則移動日曆*/';
strFrame+='{if(bDrag && window.event.button==1)';
strFrame+=' {var DateLayer=parent.document.all.meizzDateLayer.style;';
strFrame+='  DateLayer.posLeft += window.event.clientX-datelayerx;/*由於每次移動以後鼠標位置都恢復為初始的位置，因此寫法與div中不同*/';
...
strFrame+='onclick="parent.tmpSelectYearInnerHTML(this.innerText.substring(0,4))" title="點擊這裡選擇年份"><span Author=meizz id=meizzYearHead></span></td>';
...
strFrame+='        title="點擊這裡選擇月份"><span id=meizzMonthHead Author=meizz></span></td>';
...
strFrame+='             onfocus="this.blur()" title="目前日期" style="font-size: 12px; height: 20px; cursor:hand"></td><td ';
...
var outButton;  //點擊的按鈕
var outDate="";  //存放對象的日期
var odatelayer=window.frames.meizzDateLayer.document.all;  //存放日曆對像
...
document.onclick() //任意點擊時關閉該控制項 //ie6的情況可以由下面的切換焦點處理代替
{ 
  with(window.event)
...
document.onkeyup()  //按Esc鍵關閉，切換焦點關閉
  {
...
function meizzWriteHead(yy,mm)  //往 head 中寫入目前的年與月
  {
...
function meizzSetDay(yy,mm)   //主要的寫程式**********
{
  ...
  else  //本月的部分
  {
   ...
   //如果是目前選擇的日期，則顯示亮藍色的背景；如果是目前日期，則顯示暗黃色背景
   if(!outDate)
